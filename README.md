...
The tables and graph above are generated by the [`benchcmp`](benchcmp/) submodule (its own Go module, so the comparison target's dependency tree never touches this one), comparing identical operations against the widely-used [gagliardetto/solana-go](https://github.com/gagliardetto/solana-go). The same module's tests assert byte-identical output parity with upstream and run in CI. Regenerate with:
